...
var url = "http://192.168.43.188:8080/product/select/product/19/1";
mui.ajax(url, {
	...
	success: function(data) {
		...
		for (var i = 0; i < mypub.length; i++) {
			...
			html += '<div class="shopName"><img src="http://192.168.43.188:8088/image' + mypub[i].productSrcOne	 + '" /></div>'
			html += '<div class="shopContent">'
			html += '<p class="shopTitle">' + mypub[i].productName + '</p>'
			html += '<p class="colorCenter">' + mypub[i].productBigCategoryId + '     ' +mypub[i].productSmallCategoryId+ '</p>'
			html += '<div class="bottomNum">'
			html += '<div class="priceLeft"><span>￥' + mypub[i].productPrice + '</span></div>'
			html +=
				'<p class="colorCenter">' + mypub[i].productText +'</p>'
			html += '</div>'
			html += '</div>'
			html += '</li>'
			html += '</ul></div>'
