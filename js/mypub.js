	...
	success: function(data) {
		...
		if (mypub.length != 0) {
			for (var i = 0; i < mypub.length; i++) {
				...
				var categoryName = '';
				var url = "http://192.168.43.188:8080/productCategory/" + mypub[i].productSmallCategoryId;
				mui.ajax(url, {
					dataType: 'json', //服务器返回json格式数据
					type: 'get', //HTTP请求类型
					async: false,
				...
				html +=
					...
					categoryName + '</span></div><ul class="ulList">'
				...
				html +=
					'<br/><p class="colorCenter">' + mypub[i].productText + '</p>'
				html += '</div>'
				html += '<button style="margin-left:150px;" onclick="removePub(' + mypub[i].productId + ')">取消发布</button></div>'
				html += '</li>'
				html += '</ul></div>'
			}
...
function removePub(value) {
	var btnArray = ['否', '是'];
	mui.confirm('是否取消发布此闲置宝贝？', '', btnArray, function(e) {
		if (e.index == 1) {
			var url = "http://192.168.43.188:8080/product/delete/" + value;
			mui.ajax(url, {
				dataType: 'json', //服务器返回json格式数据
				type: 'get', //HTTP请求类型
				timeout: 10000, //超时时间设置为10秒；
				success: function(data) {
					mui.alert('已取消此物品发布', '嗨转小Tip', function() {
						location.reload();
					});
				},
				error: function(xhr, type, errorThrown) {
